...
echo "sfdx package version create -p PACKAGE_ID -f config/project-scratch-def.json -x -v devhub -c --json -w 50 | tee result.json"

PACKAGE_ID=$( jq -r 'first(.packageAliases[])' sfdx-project.json )

echo "Package Id: $PACKAGE_ID"

sfdx package version create -p $PACKAGE_ID -f config/project-scratch-def.json -x -v devhub -c --json -w 50 | tee result.json
